# Google-Fiber-Executive-Summary

## Executive Summary

View the interactive executive summary:

- Live (GitHub Pages): https://johngod194.github.io/Google-Fiber-Executive-Summary/
- Direct HTML file: https://johngod194.github.io/Google-Fiber-Executive-Summary/google_fiber_executive_summary.html
- Tableau Dashboard: https://public.tableau.com/app/profile/jo.o.ornelas/viz/GoogleFiberRepeatCallsDashboard_17652315144010/RepeatCallsReport

## Local Preview

To preview locally, clone the repo and open google_fiber_executive_summary.html or run a local server:

- git clone https://github.com/johngod194/Google-Fiber-Executive-Summary.git
- cd Google-Fiber-Executive-Summary
- python -m http.server 8000
- Open http://localhost:8000/google_fiber_executive_summary.html
